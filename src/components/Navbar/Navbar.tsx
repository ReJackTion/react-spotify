    ...
    <>
      {!isCollapsed ? (
        ...
        <Tooltip label={title} offset={32} position="right" withArrow>
          <div onClick={onClick}>
            <NavbarItem active={router.pathname === url}>{icon}</NavbarItem>
          </div>
        </Tooltip>
      )}
    </>
    ...
    <div className={cx(classes.wrapper, 'flex flex-col h-full')}>
      ...
      <nav className={cx(classes.navbar, 'flex-grow mx-6')}>
        <ul className="list-none">
          {getSlicedItems(0, 3)}
          <div className="mt-6">{getSlicedItems(3)}</div>
        </ul>
      ...
      {!sm && (
        <>
          <div
            className={cx(
              { 'flex flex-col justify-center': isCollapsed },
              'mx-6 my-3'
            )}
          >
            {getItem({
          ...
          <motion.div
            ...
            variants={{ initial: { height: 'auto' }, minimize: { height: 0 } }}
            animate={!isMinimized ? 'initial' : 'minimize'}
          >
