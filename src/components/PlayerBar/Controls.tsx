...
const Controls = () => {
  ...
  return (
    <div className="flex items-center flex-1 justify-end gap-4 text-[#a7a7a7]">
      ...
      <ControlButton
        tooltipLabel="Connect to a device"
        icon={<DeviceDesktop size={20} />}
      />
      ...
      {isFullscreen ? (
        <ControlButton
          icon={
            <ArrowsDiagonalMinimize2 onClick={toggleFullscreen} size={20} />
          }
        />
      ) : (
        <ControlButton
          icon={<ArrowsDiagonal onClick={toggleFullscreen} size={20} />}
        />
      )}
    </div>
  )
