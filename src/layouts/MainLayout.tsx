...
const useStyles = createStyles((theme, { sm }: { sm: boolean }) => ({
  wrapper: {
    gridTemplateAreas: `"${
      sm ? 'main-view' : 'nav-bar'
    } main-view buddy-feed" "now-playing-bar now-playing-bar now-playing-bar"`,
...
const MainLayout: FunctionComponent<{ children: ReactNode }> = ({
  children,
}) => {
  ...
  return (
    <>
      <div
        className={cx(
          classes.wrapper,
          'grid h-full min-h-full relative w-full grid-rows-1'
        )}
      >
        <Header />
        {!sm ? (
